...
export function ArticleCard({ article }) {
  ...
  return (
    <Link to={`/article/${article.id}`} className="article-card">
      <img src={article.image || 'https://images.unsplash.com/photo-1526450616598-f51cb1c26b86?auto=format&fit=crop&q=80'} alt={article.title} className="article-card-image" />
      <div className="article-card-content">
        ...
        <h3 style={{ marginBottom: 'var(--space-xs)', lineHeight: 'var(--leading-tight)' }}>{article.title}</h3>
        <p style={{ fontSize: 'var(--text-sm)', flex: 1, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {article.excerpt}
        </p>
        
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'var(--space-md)', paddingTop: 'var(--space-sm)', borderTop: '1px solid var(--border-primary)' }}>
